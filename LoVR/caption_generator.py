...
def main(args):
    ...
    for i, info in enumerate(tqdm(infos)):
        ...
        llm_inputs = []
        valid_indices = []  # keep track of which indices made it into llm_inputs

        for j, data in enumerate(process_list):
            idx, vpath = data[0], data[1]

            # ✅ Convert to absolute path & check existence
            abs_path = pathlib.Path(vpath).resolve().as_posix()
            if not os.path.exists(abs_path):
                tqdm.write(f'Error: file not found {abs_path}')
                continue

            cap = cv2.VideoCapture(abs_path)
            if not cap.isOpened():
                tqdm.write(f'error when open {abs_path}\n')
                continue

            frame_count = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
            if frame_count == 1:
                print(f'video {abs_path} is 1 frame video')
                continue

            fps = cap.get(cv2.CAP_PROP_FPS)
            if frame_count < 5:
                print(f'video {abs_path} has less than 5 frames')
                continue
